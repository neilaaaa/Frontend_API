...
import { addDemandeBrevet } from "../../features/demande/apiDemande";
import "./Demandes.css";
...
export default function RespAddDemande() {
  ...
  const updateInventeur = (i, field, val) => {
    setInventeurs((prev) => prev.map((inv, index) => (index === i ? { ...inv, [field]: val } : inv)));
  };
  ...
  const handleSubmit = async () => {
    ...
    const natureLbl = form.nature_brevet
      ? "Brevet d'invention"
      : form.nature_pct
        ? "Extension PCT"
        : form.nature_certificat
    ...
    <div className="dem-page">
      <div className="dem-card demand-form-shell">
        <div className="dem-page-header">
          <h2 className="dem-title">Nouvelle demande de protection</h2>
          <p className="dem-sub">Formulaire officiel INAPI — R2-FO-03</p>
        </div>
        ...
        <Section num="72" label="[72] — Inventeur(s)">
          {inventeurs.map((inv, i) => (
            <div key={i} className="dem-person-card">
              <div className="dem-person-header">
                <span className="dem-person-badge">Inventeur {i + 1}</span>
                {inventeurs.length > 1 && (
                  <button type="button" className="btn-cancel dem-remove-btn" onClick={() => setInventeurs(inventeurs.filter((_, j) => j !== i))}>
                    Supprimer
                  </button>
                )}
              </div>
          ...
          <button type="button" className="dem-add-btn" onClick={() => setInventeurs([...inventeurs, { nom_inv: "", prenom_inv: "", adress_inv: "" }])}>
...
function F({ label, name, value, onChange, type = "text", full, area, rows = 2 }) {
  return (
    <div className={`fg${full ? " full" : ""}`}>
      <label>{label}</label>
      {area ? <textarea name={name} value={value ?? ""} onChange={onChange} rows={rows} /> : <input type={type} name={name} value={value ?? ""} onChange={onChange} />}
    </div>
  );
}
