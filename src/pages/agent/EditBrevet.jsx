...
export default function EditBrevet() {
  ...
  useEffect(() => {
    const fetchBrevet = async () => {
      try {
        ...
        const documents = await getDocumentsByBrevet(id);
        setDocs(documents.results ?? documents);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (data?.id_demande) {
        window.alert("Cette demande est deja attribuee a un autre brevet. Veuillez en choisir une autre.");
      } else {
  ...
  const handleUpdateDoc = async (docId, updatedDoc) => {
    try {
      if (updatedDoc.fichier instanceof File) {
        const formData = new FormData();
        formData.append("nom_document", updatedDoc.nom_document);
  ...
  const handleDeleteDoc = async (docId) => {
    try {
      ...
      setDocs(docs.filter((doc) => doc.id_document !== docId));
    } catch {
  ...
  const demandeActuelle = demande.find(
    (currentDemande) => String(currentDemande.id_demande) === String(form?.id_demande)
  );
    ...
    <div className="brevet-page">
      <div className="brevet-card">
        ...
        <div className="brevet-grid">
          ...
          <div className="form-group">
            ...
            <div className="demande-field">
              ...
              >
                <option value=""></option>
                {demande.map((currentDemande) => (
                  <option key={currentDemande.id_demande} value={currentDemande.id_demande}>
                    {currentDemande.titre}
                  </option>
                ))}
              </select>
              ...
              {demandeActuelle && (
                <button
                  type="button"
                  className="btn-link"
                  onClick={() => navigate(`/agent/demandes/${demandeActuelle.id}`)}
                >
                  Voir la demande
                </button>
              )}
        ...
        <div className="container_btn">
          <button className="btn-save" type="submit" onClick={handleSubmit} disabled={saving}>
